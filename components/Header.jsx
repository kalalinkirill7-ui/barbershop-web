  ...
  return (
    <header className="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-stone-200/60 shadow-sm">
      <div className="max-w-6xl mx-auto px-5 py-3 flex justify-between items-center">
        ...
        {/* Гамбургер */}
        <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden text-stone-600 hover:text-stone-900 p-2">
      ...
      {/* Мобильное меню */}
      {menuOpen && (
        <nav className="md:hidden bg-white border-t border-stone-200 px-5 py-5 flex flex-col gap-4 text-lg font-medium animate-in slide-in-from-top">
          {isAdmin ? (
            <>
              <Link href="/admin/services" className="text-stone-600" onClick={() => setMenuOpen(false)}>Услуги</Link>
              <Link href="/admin/barbers" className="text-stone-600" onClick={() => setMenuOpen(false)}>Мастера</Link>
              <Link href="/admin/appointments" className="text-stone-600" onClick={() => setMenuOpen(false)}>Записи</Link>
              <Link href="/admin/content" className="text-stone-600" onClick={() => setMenuOpen(false)}>Контент</Link>
              <Link href="/admin/gallery" className="text-stone-600" onClick={() => setMenuOpen(false)}>Галерея</Link>
              <Link href="/admin/reviews" className="text-stone-600" onClick={() => setMenuOpen(false)}>Отзывы</Link>
            </>
          ) : (
            <>
              ...
              <Link href="/my-bookings" className="text-stone-600" onClick={() => setMenuOpen(false)}>Мои записи</Link>
              {user ? (
                <>
                  <Link href="/profile" className="text-stone-600" onClick={() => setMenuOpen(false)}>{user.user_metadata?.name || user.email}</Link>
                  <button onClick={() => { handleLogout(); setMenuOpen(false) }} className="text-red-400 text-left">Выйти</button>
                </>
              ) : (
                <>
                  <Link href="/login" className="text-stone-600" onClick={() => setMenuOpen(false)}>Войти</Link>
                  <Link href="/register" className="text-stone-600" onClick={() => setMenuOpen(false)}>Регистрация</Link>
                </>
              )}
            </>
          )}
